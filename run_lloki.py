...
if __name__ == "__main__":
    ...
    parser.add_argument('--alpha', type=float, default=0.05, help="Alpha parameter for soft feature propagation")
    ...
    parser.add_argument('--pretrain_epochs', type=int, default=0, help="Number of epochs to train with only the autoencoder and neighborhood-preservation losses before introducing the triplet loss")
    parser.add_argument('--ramp_up_epochs', type=int, default=10, help="Number of epochs to linearly ramp up the triplet loss weight after pretraining")
    parser.add_argument('--cell_type_informed_triplet', action=argparse.BooleanOptionalAction, default=True, help="Use cell-type-informed positive/negative sampling for the triplet loss (paper default). Pass --no-cell_type_informed_triplet to fall back to plain mutual-NN sampling with random negatives")
